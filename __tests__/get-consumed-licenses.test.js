...
describe('UserManager', () => {
  ...
  it('should fetch all consumed copilot licenses in an enterprise successfully', async () => {
    ...
    expect(hold_until_rate_limit_success).toHaveBeenCalledWith(13, token, headers)
  })
  ...
  it('should handle errors when fetching onsumed copilot licenses in an enterprise', async () => {
    ...
    await expect(
      userManager.getConsumedLicenses(ent)
  ...
  it('should perform rate limit check if there are more pages', async () => {
    ...
    octokit.paginate.iterator.mockReturnValue({
      async *[Symbol.asyncIterator]() {
        ...
        while (seatsReturned < seatsCount) {
          let seats = (seatsCount - seatsReturned) > 100 ? 100 : (seatsCount - seatsReturned)
          seatsReturned += seats
    ...
    expect(hold_until_rate_limit_success).toHaveBeenCalledWith(37, token, headers)
